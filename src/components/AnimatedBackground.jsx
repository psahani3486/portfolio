...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    const animate = (currentTime) => {
      ...
      // Aurora blob 1
      const a1x = width * (0.3 + Math.sin(time * 0.4) * 0.12 + mx * 0.03)
      const a1y = height * (0.25 + Math.cos(time * 0.3) * 0.08 + my * 0.02)
      ...
      // Aurora blob 2
      const a2x = width * (0.7 + Math.cos(time * 0.35) * 0.1 - mx * 0.03)
      const a2y = height * (0.7 + Math.sin(time * 0.25) * 0.1 - my * 0.02)
      const grad2 = ctx.createRadialGradient(a2x, a2y, 0, a2x, a2y, width * 0.35)
      grad2.addColorStop(0, 'rgba(168, 85, 247, 0.06)')
      grad2.addColorStop(0.5, 'rgba(168, 85, 247, 0.02)')
      grad2.addColorStop(1, 'transparent')
      ctx.fillStyle = grad2
      ctx.fillRect(0, 0, width, height)

      // Aurora blob 3 (cyan accent)
      const a3x = width * (0.5 + Math.sin(time * 0.4 + 2) * 0.15)
      const a3y = height * (0.5 + Math.cos(time * 0.2 + 1) * 0.1)
      const grad3 = ctx.createRadialGradient(a3x, a3y, 0, a3x, a3y, width * 0.25)
      grad3.addColorStop(0, 'rgba(34, 211, 238, 0.035)')
